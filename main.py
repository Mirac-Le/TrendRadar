...
def send_to_feishu(
    webhook_url_input: Union[str, List[str]], # 修改参数名为 input，类型支持 str 或 List
    report_data: Dict,
    report_type: str,
    update_info: Optional[Dict] = None,
    proxy_url: Optional[str] = None,
    ...
    # === 1. 解析 Webhook URL 列表 ===
    target_urls = []
    if isinstance(webhook_url_input, list):
        target_urls = webhook_url_input
    elif isinstance(webhook_url_input, str):
        # 支持使用逗号或分号分隔多个 URL
        normalized_str = webhook_url_input.replace(";", ",")
        target_urls = [url.strip() for url in normalized_str.split(",") if url.strip()]
    
    if not target_urls:
        print(f"❌ 飞书 Webhook URL 配置为空")
        return False
    ...
    print(f"飞书消息准备完毕，分为 {len(batches)} 批次，将推送到 {len(target_urls)} 个群")

    # === 3. 遍历所有群组 URL 发送 ===
    all_success = True

    for group_idx, webhook_url in enumerate(target_urls, 1):
        print(f"--- 正在推送第 {group_idx}/{len(target_urls)} 个飞书群 ---")
        ...
        for i, batch_content in enumerate(batches, 1):
            ...
            # 添加批次标识
            current_batch_content = batch_content
            if len(batches) > 1:
                batch_header = f"**[第 {i}/{len(batches)} 批次]**\n\n"
                if "📊 **热点词汇统计**" in current_batch_content:
                    current_batch_content = current_batch_content.replace(
                        "📊 **热点词汇统计**\n\n", f"📊 **热点词汇统计** {batch_header}"
                    )
                else:
                    current_batch_content = batch_header + current_batch_content
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    if result.get("StatusCode") == 0 or result.get("code") == 0:
                        ...
                        error_msg = result.get("msg") or result.get("StatusMessage", "未知错误")
                        print(
                            f"  ❌ 群{group_idx} 批次{i} 发送失败: {error_msg}"
                        )
                        all_success = False
                else:
                    print(
                        f"  ❌ 群{group_idx} 批次{i} HTTP错误: {response.status_code}"
                    )
                    all_success = False
            except Exception as e:
                print(f"  ❌ 群{group_idx} 批次{i} 异常: {e}")
                all_success = False
        
        # 群之间稍微停顿一下，防止触发 API 频率限制
        if group_idx < len(target_urls):
            time.sleep(1)

    if all_success:
        print(f"飞书所有群组推送完成 [{report_type}]")
    else:
        print(f"飞书推送完成，但部分群组或批次存在失败 [{report_type}]")
